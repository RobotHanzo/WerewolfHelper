...
export const SpectatorView: React.FC<SpectatorViewProps> = ({players, doubleIdentities}) => {
    ...
    const stats = useMemo(() => {
        ...
        players.forEach(player => {
            ...
            pRoles.forEach(role => {
                ...
                const isWolf = role.includes('狼') || ['WEREWOLF', 'GARGOYLE', 'BLOOD_MOON', 'NIGHTMARE', 'WOLF_KING', 'WHITE_WOLF_KING', 'WOLF_BROTHER', 'HIDDEN_WOLF'].includes(uRole);
    ...
    const winProgressWolves = (stats.gods + stats.villagers) > 0 ? (((stats.deadGods) + (stats.deadVillagers)) / (stats.gods + stats.villagers)) * 100 : 0;
        ...
        <div className="text-white font-['Spline_Sans']">
            ...
            <div className="grid grid-cols-12 gap-12">
                ...
                <aside className="col-span-3 space-y-6">
                    <div
                        className="flex items-center gap-2 text-slate-400 font-black text-sm uppercase tracking-[0.3em] mb-4">
                    ...
                    <div className="pt-4">
                        <div className="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/10 space-y-3">
                            <div className="flex items-center gap-2 text-indigo-400">
                                ...
                                <span
                                    className="font-black text-sm uppercase tracking-wider">{t('spectator.winConditions')}</span>
                            </div>
                ...
                <main className="col-span-9 space-y-12">
                    {/* Win Tracking Banner */}
                    <section
                        className="bg-slate-900/30 border border-slate-800/50 rounded-3xl p-10 relative overflow-hidden backdrop-blur-md">
                        <div className="flex items-center justify-between gap-12">
                            {/* Good Faction Progress */}
                            <div className="flex-1 space-y-5">
                                <div
                                    className="flex items-center justify-between text-xl font-black tracking-widest text-blue-400 uppercase">
                                    <span>{(doubleIdentities && stats.jbbs > 0) ? t('status.jinBaoBao') : t('roles.factions.civilian')} & {t('roles.factions.special')}</span>
                            ...
                            <div className="flex-1 space-y-5">
                                <div
                                    className="flex items-center justify-between text-xl font-black tracking-widest text-red-400 uppercase">
                    ...
                    <section className="space-y-6">
                        <div className="flex items-center gap-4">
                            <div
                                className="flex items-center gap-2 text-slate-400 font-bold text-lg uppercase tracking-[0.2em]">
        ...
        <div
            ...
            <div className="flex items-center justify-between mb-2 relative z-10">
                <div className="flex items-center gap-3">
                    <div
                        className="p-2 rounded-lg bg-slate-950/50 border border-white/5 transition-transform group-hover:scale-110 duration-500">
                        {icon}
                    </div>
...
const SpectatorPlayerCard: React.FC<{ player: Player }> = ({player}) => {
    ...
    const isWolf = role.includes('狼') || ['WEREWOLF', 'GARGOYLE', 'BLOOD_MOON', 'NIGHTMARE', 'WOLF_KING', 'WHITE_WOLF_KING', 'WOLF_BROTHER', 'HIDDEN_WOLF'].includes(uRole);
    const isGod = ['WITCH', 'SEER', 'HUNTER', 'GUARD', 'IDIOT', 'KNIGHT', 'GRAVE_KEEPER', 'DREAM_EATER', 'MAGICIAN'].includes(uRole) || (role !== '平民' && uRole !== 'VILLAGER' && !isWolf);
        ...
        <div
            ...
            <div className="relative z-10 flex flex-col items-center gap-3">
                ...
                <div className="relative">
                    ...
                    {!isDead && (
                        <div
                            className="absolute top-0 right-0 w-3 h-3 bg-emerald-500 border-2 border-[#1A1C23] rounded-full shadow-lg"/>
                    )}
                </div>
