...
export const PlayerEditModal: React.FC<PlayerEditModalProps> = ({
  ...
  const [rolePositionLocked, setRolePositionLocked] = useState<boolean>(
    player.rolePositionLocked || false
  );

  // Default to session setting if available, otherwise fallback to player data
  const isDoubleIdentity =
    doubleIdentities !== undefined ? doubleIdentities : player.roles.length > 1;
            ...
            <div className="space-y-3 pt-3 border-t border-slate-200 dark:border-slate-800">
              ...
              <div className="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-900/30 rounded-lg p-4">
                <p className="text-sm text-slate-600 dark:text-slate-400 mb-3">
                  {t(
                    'players.transferPoliceDescription',
                    'Transfer the police badge to another alive player.'
                  )}
                </p>
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    onChange={(e) =>
                      setTransferTarget(e.target.value === '' ? '' : Number(e.target.value))
                    }
                  >
