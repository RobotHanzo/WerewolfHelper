  ...
  return (
    <div className="flex flex-col gap-12 w-full animate-in fade-in duration-500 overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
            {t(
              'vote.statusDescription',
              'Live tally of village consensus and player participation'
            )}
          </p>
        </div>
        {endTime && (
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          ...
          <div className="flex flex-col items-end gap-2 w-full sm:w-auto sm:min-w-[200px]">
            <div className="flex justify-between w-full text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 gap-4">
            ...
            <div className="h-2 w-full bg-slate-100 dark:bg-white/10 rounded-full overflow-hidden p-[1px] border border-slate-200 dark:border-white/5">
              <div
                ...
                style={{
                  width: `${totalVoters ? Math.min(100, (totalVotes / totalVoters) * 100) : 0}%`,
                }}
              />
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {sortedCandidates.length === 0 ? (
            <div className="col-span-full py-16 bg-slate-50 dark:bg-white/5 border border-dashed border-slate-300 dark:border-white/10 rounded-2xl flex flex-col items-center justify-center text-slate-400 text-sm italic">
            ...
            sortedCandidates.slice(0, 4).map((candidate, idx) => {
              ...
              const isLeading =
                candidate.voters.length === maxVoteCount && candidate.voters.length > 0;
              ...
              return (
                ...
                >
                  {isLeading && (
                    <div className="absolute -inset-0.5 bg-[#3211d4]/20 rounded-2xl blur opacity-30 group-hover:opacity-60 transition duration-1000 candidate-glow"></div>
                  )}
                  <div
                  ...
                  >
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex gap-5 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-3 flex-nowrap min-w-0">
                            ...
                            {isLeading && (
                              <span className="px-2 py-0.5 bg-[#3211d4] text-[10px] font-black uppercase rounded tracking-tighter text-white flex-shrink-0">
                      ...
                      <div className="text-right flex-shrink-0">
                        <div className="text-5xl font-black text-[#3211d4] dark:text-indigo-400 leading-none drop-shadow-sm">
                          {candidate.voters.length}
                        </div>
                        <div className="text-[10px] font-bold text-slate-500 uppercase mt-2 tracking-widest">
                    ...
                    <div className="space-y-2.5">
                      <div className="flex justify-between text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                        ...
                        <span className={isLeading ? 'text-[#3211d4] dark:text-indigo-400' : ''}>
                          {Math.round(voteRatio)}%
                        </span>
                      </div>
                      <div className="h-2.5 w-full bg-slate-100 dark:bg-white/10 rounded-full overflow-hidden p-[1px]">
                        <div
      ...
      <div className="space-y-8 pt-4">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between border-b border-slate-200 dark:border-white/10 pb-4 gap-4">
          <div className="min-w-0">
            <h3 className="text-lg font-black text-slate-900 dark:text-white tracking-tight uppercase">
              {t('vote.votingMap', 'Voting Map')}
            </h3>
          </div>
          <div className="flex flex-wrap gap-x-6 gap-y-2 text-[10px] font-black uppercase tracking-wider text-slate-500">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="size-2.5 rounded-full bg-[#3211d4] dark:bg-indigo-400"></span>{' '}
              {t('vote.voted', 'Voted')}
            </div>
            <div className="flex items-center gap-2 mb-0.5">
              <span className="size-2.5 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]"></span>{' '}
              {t('vote.thinking', 'Thinking')}
            </div>
            <div className="flex items-center gap-2 mb-0.5 opacity-50">
              <span className="size-2.5 rounded-full bg-slate-200 dark:bg-slate-700"></span>{' '}
              {t('vote.ineligible', 'Ineligible')}
            </div>
            <div className="flex items-center gap-2 mb-0.5 opacity-50">
              <span className="size-2.5 rounded-full bg-slate-400 dark:bg-slate-800"></span>{' '}
              {t('vote.eliminated', 'Eliminated')}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 xl:grid-cols-8 gap-4">
          {players.map((player) => {
            ...
            return (
              <div
                key={player.id}
                className={`bg-white dark:bg-slate-900/50 border transition-all duration-300 rounded-xl p-3 flex flex-col items-center gap-2 relative group shadow-sm overflow-hidden min-w-0
                                ${
                                  isDead
                                    ? 'opacity-40 grayscale border-slate-100 dark:border-white/5'
                                    : isIneligible
                                      ? 'opacity-50 grayscale bg-slate-50 dark:bg-white/5 border-slate-200 dark:border-white/5'
                                      : votedFor
                                        ? 'border-[#3211d4]/30 hover:bg-[#3211d4]/5 select-none'
              ...
              >
                  ...
                  className={`size-14 rounded-xl overflow-hidden transition-all duration-500
                                    ${
                                      votedFor
                                        ...
                                        : isThinking
                                          ? 'ring-2 ring-amber-500/20 animate-pulse'
                                          : 'bg-slate-900'
                                    }`}
                >
                  {isDead ? (
                    <div className="size-full bg-slate-200 dark:bg-slate-950 flex items-center justify-center">
                ...
                {isDead ? (
                  <span className="text-[9px] px-2 py-0.5 rounded-full bg-red-950/20 text-red-500 border border-red-900/10 font-black uppercase tracking-tighter">
                    {t('dashboard.eliminated', 'DEAD')}
                  </span>
                ) : isIneligible ? (
                  <span className="text-[9px] px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 border border-slate-200 dark:border-slate-700 font-black uppercase tracking-tighter">
                    {t('vote.ineligible', 'INELIGIBLE')}
                  </span>
                ) : votedFor ? (
                  <span className="text-[9px] px-2 py-0.5 rounded-full bg-[#3211d4]/10 dark:bg-indigo-400/10 text-[#3211d4] dark:text-indigo-400 border border-[#3211d4]/20 dark:border-indigo-400/20 font-black uppercase tracking-tighter truncate max-w-full">
                    <DiscordName
                      userId={String(votedFor.userId || '')}
                      guildId={guildId}
                      fallbackName={votedFor.nickname}
                    />
                  </span>
                ) : (
                  <span className="text-[9px] px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-500 border border-amber-500/20 font-black uppercase tracking-tighter animate-pulse">
