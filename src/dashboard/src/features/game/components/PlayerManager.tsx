  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-sans flex flex-col md:flex-row overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        {showTimerModal && (
          <TimerControlModal onClose={() => setShowTimerModal(false)} onStart={handleTimerStart} />
        )}
        ...
        {playerSelectModal.visible && (
          <PlayerSelectModal
            title={
              playerSelectModal.type === 'ASSIGN_JUDGE'
                ? t('modal.assignJudge')
                : playerSelectModal.type === 'DEMOTE_JUDGE'
                  ? t('modal.demoteJudge')
                  : playerSelectModal.type === 'FORCE_POLICE'
                    ? t('modal.forcePolice')
                    : playerSelectModal.type === 'ADD_SPECTATOR'
                      ? t('modal.addSpectator')
                      : playerSelectModal.type === 'REMOVE_SPECTATOR'
                        ? t('modal.removeSpectator')
                        : ''
            }
