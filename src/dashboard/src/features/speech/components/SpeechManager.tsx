...
export const SpeechManager = ({
  ...
  const isSelectingOrder = !!(
    speech &&
    (currentSpeakerId === -1 || currentSpeakerId == null) &&
    (!speech.order || speech.order.length === 0)
  );

  // Core activity flags
  const isSpeechRunning =
    speech &&
    ((currentSpeakerId != null && currentSpeakerId !== 0) ||
      (speech.order && speech.order.length > 0) ||
      isSelectingOrder);
              ...
              <div key={`${pid}-${idx}`} className="relative w-full">
                ...
                <div className="w-full bg-slate-50 dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center justify-between opacity-70">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="w-6 h-6 flex-shrink-0 flex items-center justify-center bg-slate-200 dark:bg-slate-800 rounded-full text-xs font-bold text-slate-500">
                      {idx + 1}
                    </span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!readonly &&
                      police?.state === 'SPEECH' &&
                      activeCandidates.some((c) => c.id === pid) && (
                        <button
  ...
  if (police?.state === 'VOTING') {
    const eligibleVoters = players
      .filter((p) => p.alive && !activeCandidates.some((c) => c.id === p.id))
      .map((p) => p.id);
    return (
      <div className="h-full flex flex-col p-4 gap-6 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 items-center justify-center">
        <div className="w-full max-w-2xl bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 border border-slate-200 dark:border-slate-700">
          <VoteStatus
  ...
  if (police?.state === 'ENROLLMENT' || police?.state === 'UNENROLLMENT') {
    ...
    return (
      <div className="h-full flex flex-col p-4 gap-8 overflow-y-auto scrollbar-hide animate-in fade-in duration-500">
        ...
        >
          <div
            className={`absolute inset-0 bg-gradient-to-r via-transparent to-transparent ${isUnenrollment ? 'from-amber-500/10' : 'from-indigo-500/10'}`}
          ></div>

          <div className="p-6 md:p-8 relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-6">
              <div
                className={`h-16 w-16 rounded-2xl flex items-center justify-center border shadow-lg animate-in fade-in zoom-in-75 duration-500 delay-100 fill-mode-both
                                ${
                                  isUnenrollment
              ...
              <div className="animate-in fade-in slide-in-from-left-4 duration-500 delay-200 fill-mode-both">
                <h2
                  className={`font-bold text-sm tracking-widest uppercase mb-1 ${isUnenrollment ? 'text-amber-600' : 'text-indigo-600'}`}
                >
                  {isUnenrollment
                    ? t('speechManager.policeUnenrollment')
                ...
                <div className="flex items-center gap-4 mt-3">
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${police?.allowEnroll ? 'bg-green-500' : 'bg-red-500'}`}
                    ></div>
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-tighter">
                      {t('speechManager.allowEnroll')}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${police?.allowUnEnroll ? 'bg-green-500' : 'bg-red-500'}`}
                    ></div>
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-tighter">
                      {t('speechManager.allowUnEnroll')}
                    </span>
                  </div>
                </div>
              </div>
            ...
            {police?.stageEndTime && (
              <div className="flex-shrink-0 animate-in fade-in slide-in-from-right-4 duration-500 delay-300 fill-mode-both">
                <Timer
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between px-2">
            <h3 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-3">
              ...
              <span className="ml-2 px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-xs font-bold text-slate-500">
          ...
          {activeCandidates.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 px-4 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-white/[0.02] animate-in fade-in duration-700">
              <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center mb-4 text-slate-300 dark:text-slate-700">
                <UserMinus className="w-8 h-8" />
              </div>
              <h4 className="text-lg font-bold text-slate-400 dark:text-slate-600">
                {t('speechManager.noCandidates')}
              </h4>
              <p className="text-sm text-slate-400 mt-1">
                {t(
                  'speechManager.waitingForCandidates',
                  'Expecting participants to step forward...'
                )}
              </p>
            </div>
          )}
          ...
          <div className="flex flex-col items-center gap-6 animate-in fade-in zoom-in-95">
            ...
            {!readonly && (
              <div className="flex flex-col gap-3 w-full max-w-xs">
                <div className="text-xs text-center text-slate-400 uppercase font-bold tracking-wider mb-2">
              ...
              {currentSpeaker ? (
                ...
                >
                  <SpeakerCard
                    ...
                    onUnenroll={
                      police?.state === 'SPEECH' &&
                      activeCandidates.some((c) => c.id === currentSpeaker.id)
                        ? () => handleUnenroll(currentSpeaker.id)
                        : undefined
                    }
                  />
                </div>
            ...
            {speech?.interruptVotes && speech.interruptVotes.length > 0 && (
              <div className="w-full max-w-md bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded-lg p-4 animate-in slide-in-from-bottom-2 duration-300">
                <h4 className="text-sm font-bold text-red-700 dark:text-red-400 mb-2 flex items-center gap-2">
                  ...
                  {t('speechManager.interruptVote')} ({speech.interruptVotes.length} /{' '}
                  {Math.floor(players.filter((p) => p.alive).length / 2) + 1})
                </h4>
