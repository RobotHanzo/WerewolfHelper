        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              {player.isSheriff && (
                <div className="absolute -bottom-1 -right-1 bg-green-600 text-white rounded-full p-0.5 border-2 border-slate-200 dark:border-slate-800" title={t('status.sheriff')}>
              ...
              {player.roles.length > 1 && !player.rolePositionLocked && (
                <div className="absolute -top-1 -left-1 bg-amber-500 text-white rounded-full p-0.5 border-2 border-slate-200 dark:border-slate-800 z-10" title="Role Position Unlocked">
              ...
              {showLock && (
                <div className={`absolute -top-1 -left-1 bg-indigo-600 text-white rounded-full p-0.5 border-2 border-slate-200 dark:border-slate-800 z-20 transition-opacity duration-1000 ${fading ? 'opacity-0' : 'opacity-100'}`} title="Role Position Locked">
            ...
            <div>
              ...
              <div className={`flex items-center gap-1.5 mt-1`}>
                <div className={`flex items-center gap-1.5 transition-all ${animate ? 'animate-flash' : ''}`}>
                  {!player.avatar && (
                  ...
                  {player.roles && player.roles.length > 0 && player.roles.map((role, index) => {
                    ...
                    return (
                      <span
                        key={`${index}-${role}`}
                        onClick={() => !readonly && isDeadRole ? onAction(player.id, `revive_role:${role}`) : undefined}
