  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center transition-all duration-300 ${
        isAnimating ? 'bg-slate-900/80 backdrop-blur-sm' : 'bg-slate-900/0'
      }`}
    >
      <div
        className={`bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700 m-4 flex flex-col max-h-[80vh] transition-all duration-300 ${
              ...
              <div className="space-y-2">
                {logs.map((log, index) => (
                  <div key={index} className="flex gap-3 text-slate-300">
                    ...
                    <span
                      className={
                        log.includes('錯誤') || log.includes('Failed')
                          ? 'text-red-400'
                          : log.includes('完成') || log.includes('Success')
                            ? 'text-emerald-400'
                            : 'text-slate-300'
                      }
                    >
