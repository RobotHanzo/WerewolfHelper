...
export const ProgressOverlay: React.FC<ProgressOverlayProps> = ({
    ...
    return (
        <div className={`fixed inset-0 z-50 flex items-center justify-center transition-all duration-300 ${isAnimating ? 'bg-slate-900/80 backdrop-blur-sm' : 'bg-slate-900/0'
            }`}>
            <div className={`bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700 m-4 flex flex-col max-h-[80vh] transition-all duration-300 ${isAnimating ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
                }`}>

                {/* Header */}
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center gap-4 bg-slate-50 dark:bg-slate-950/50">
                    ...
                    <div className="flex-1">
                        ...
                        {status === 'success' && <p className="text-sm text-emerald-600 dark:text-emerald-400">{t('progressOverlay.complete')}</p>}
                        {status === 'error' && <p className="text-sm text-red-600 dark:text-red-400">{error || t('progressOverlay.unknownError')}</p>}
                    </div>
                </div>

                {/* Log Terminal */}
                <div className="flex-1 overflow-y-auto p-4 bg-slate-950 font-mono text-sm relative min-h-[200px]">
