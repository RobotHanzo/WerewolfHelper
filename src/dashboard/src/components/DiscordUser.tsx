...
export const DiscordAvatar: React.FC<DiscordAvatarProps> = ({
  ...
  const shouldUseInitialsFallback = useInitialsFallback && !userId && !cachedUser?.avatar;
  ...
  return <img src={avatarUrl} alt={name} className={avatarClassName} />;
};

export const DiscordName: React.FC<DiscordNameProps> = ({
  userId,
  guildId,
  fallbackName,
  nameClassName,
