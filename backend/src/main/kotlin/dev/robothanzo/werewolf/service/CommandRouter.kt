...
/**
 * The single [DiscordCommandHandler] the gateway knows about. Slash commands fan out to the service
 * that owns each one: `/server` lifecycle → [ServerProvisioningService], `/game` gameplay actions →
 * [DayOrchestrator]. Mirrors [InteractionRouter] for component interactions, and keeps each service
 * focused on its own concern instead of all implementing one fat handler. Registers itself in
 * `@PostConstruct`, avoiding a constructor cycle with the gateway.
 */
@Component
class CommandRouter(
