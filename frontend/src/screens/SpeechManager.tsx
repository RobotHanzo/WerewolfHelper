      ...
      <AnimatePresence mode="wait">
        {speech?.active && speech.speakerSeat != null && (
          ...
          >
            ...
            {!readOnly && (
              <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 10, marginTop: 6 }}>
                {/* 下台: judge override that forces the current speaker off (advance). Skipping a turn
                    is a player self-action done over Discord, so it isn't surfaced on the judge console. */}
                <Button variant="danger" onClick={actions.skipSpeech}>{t("speech.stepDown")}</Button>
